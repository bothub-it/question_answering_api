...
def split_train_test(train_path, eval_percentage=0.15, format_data=True):
...
def send_job_train_ai_platform(train_data, model_type, model_name, config_data, eval_data=None, eval_percentage=0.15):
    ...
    if not bucket.blob(posixpath.join(job_id, 'train_data.json')).exists():
        if eval_data is None:
            split_train_test(train_path, eval_percentage)
            bucket.blob(posixpath.join(job_id, 'train_data.json')).upload_from_filename('data/train_data.json')
            bucket.blob(posixpath.join(job_id, 'eval_data.json')).upload_from_filename('data/eval_data.json')
        else:
    ...
    job_spec = {"jobId": job_id.replace('-', '_') + datetime.now().strftime("_%d_%m_%Y_%H_%M"), "trainingInput": training_inputs}
...
if __name__ == '__main__':
    ...
    send_job_train_ai_platform(train_data, model_type, model_name, config_data)
