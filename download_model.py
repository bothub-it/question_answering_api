...
def find_occurrences(
    s, ch
):  # to find position of '/' in blob path ,used to create folders in local storage
            ...
            if blob.name.replace(bucket_path, "").find("/") == -1:
                ...
                for folder in folder_loc:
                    if not os.path.exists(
                        local_path
                        + "/"
                        + blob.name.replace(bucket_path, "")[start_loc:folder]
                    ):
                        create_folder = (
                            local_path
                            + "/"
                            + blob.name.replace(bucket_path, "")[0:start_loc]
                            + "/"
                            + blob.name.replace(bucket_path, "")[start_loc:folder]
                        )
                        start_loc = folder + 1
                ...
                print(
                    blob.name.replace(bucket_path, "")[
                        0 : blob.name.replace(bucket_path, "").find("/")
                    ]
                )
...
def download_model(model):
    ...
    storage_client = storage.Client(
        project=settings.BOTHUB_GOOGLE_PROJECT_ID, credentials=credentials
    )
    ...
    download_folder_structure_from_bucket(
        bucket, model, model_info.get(settings.model).get("bucket_path")
    )
...
if __name__ == "__main__":
    if settings.model:
        ...
        print(
            "no language selected, use --lang or set env var MODEL to your desired language"
        )
